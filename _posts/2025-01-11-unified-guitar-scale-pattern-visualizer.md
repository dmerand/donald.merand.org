---
layout: post
title: Unified Stringed Instrument Scale Pattern Visualizer
category: projects
---

<div class="text-sm">
  <p>
    <em>Updated 2026/05/25: Added mandolin, banjo (open G and open C), and 12-string perfect-fifths tunings.</em>
  </p>
  <p>
...
<option value="perfect-fifths">12-String Perfect 5ths</option>
<option value="standard-guitar">6-String Guitar</option>
<option value="mandolin">Mandolin</option>
<option value="banjo-open-g">Banjo (Open G)</option>
<option value="banjo-open-c">Banjo (Open C)</option>
...
<p>Version 1.4.0 • Last updated: May 25, 2026 • <a href="https://github.com/dmerand/donald.merand.org/tree/master/lib/unified-nps" class="text-gray-500 hover:text-gray-700 underline">Source code</a></p>
</div>
 ...
 * Version: 1.4.0
 * Built: 2026-05-26T00:48:41.423Z
...
class FretboardAlgorithm {
  ...
  static get TUNING_PRESETS() {
    return {
      ...
      'perfect-fifths': ['Eb1', 'Bb1', 'F2', 'C3', 'G3', 'D4', 'A4', 'E5', 'B5', 'F#6', 'C#7', 'G#7'],
      'standard-guitar': ['E2', 'A2', 'D3', 'G3', 'B3', 'E4'],
      'mandolin': ['G3', 'D4', 'A4', 'E5'],
      'banjo-open-g': ['D3', 'G3', 'B3', 'D4', 'G4'],
      'banjo-open-c': ['G3', 'C4', 'E4', 'G4', 'C5'],
...
class ScaleVisualizer {
  ...
  generateVisualizationData(config) {
    ...
    // Apply octave optimization for root note based on tuning range
    const optimizedRootNote = this.getOptimalRootNote(rootNote, tuning);
   ...
   */
  getOptimalRootNote(rootNote, tuning) {
    const lowestOpenSemitone = this.musicalTheory.parseNote(tuning[0]);
    const noteValue = this.musicalTheory.noteValues[rootNote.replace(/[#b]/, '')] +
      (rootNote.includes('#') ? 1 : rootNote.includes('b') ? -1 : 0);
    const lowestOctave = Math.floor(lowestOpenSemitone / 12);
    const candidate = noteValue + lowestOctave * 12;
    const octave = candidate < lowestOpenSemitone ? lowestOctave + 1 : lowestOctave;
