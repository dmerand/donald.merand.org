...
function updateBlogPost(newScriptContent) {
  ...
  const marker = '<!-- banjo-chord-script -->';
  const markerIdx = blogContent.indexOf(marker);
  if (markerIdx === -1) {
    throw new Error('Could not find banjo-chord-script marker in blog post');
  }
  const scriptStart = blogContent.indexOf('<script>', markerIdx);
  const scriptEnd = blogContent.indexOf('</script>', markerIdx) + '</script>'.length;

  if (scriptStart === -1 || scriptEnd === -1) {
    throw new Error('Could not find <script> tags after marker in blog post');
  }
